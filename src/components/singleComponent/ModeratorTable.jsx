...
const ModeratorTable = () => {
  ...
  const fetchModerators = async () => {
    try {
      const response = await fetch(`http://89.116.159.133:8008/api/get-all/moderator?page=1&limit=10`);
  ...
  const handleDeleteModerator = async (moderatorId) => {
    ...
    try {
      const response = await fetch(`http://89.116.159.133:8008/api/delete/moderator?id=${moderatorId}`, {
        method: 'DELETE',
      });
      const data = await response.json();
      if (response.ok) {
  ...
  const handleInviteModerator = async () => {
    try {
      const response = await fetch(`http://89.116.159.133:8008/api/moderator-invitation/link'`, {
        method: 'POST',
        headers: {
