...
const Step5 = ({ formData, setFormData }) => {
  ...
  const removePool = (index) => {
    const updatedPool = formData.pools.filter(
      (_, i) => i !== index
    );
  ...
  const activePoolsCount = formData?.pools?.filter(pool => pool.active).length ?? 0;
  ...
  return (
    <div>
      ...
      <FormDropdownLabel
          children={`You have ${activePoolsCount} active poll${activePoolsCount !== 1 ? 's' : ''} for this meeting.`}
      ...
      {formData?.pools?.map((pool, index) => (
        <div key={index} className="py-3">
          <div className="flex justify-start items-center bg-white rounded-xl shadow-[0px_0px_6px_#00000029] p-3">
            ...
            <ParagraphLg className='w-[20%]'>{`${pool.questions.length} ${pool.questions.length > 1 ? 'Questions' : 'Question'}`}</ParagraphLg>


            <ParagraphLg className='w-[20%]'>{pool.creator}</ParagraphLg>
            <ParagraphLg className='w-[30%]'>{pool.active ? <span>Active</span> : <span>Inctive</span>}</ParagraphLg>
