          ...
          <div className="flex justify-start items-center gap-5 w-full">
            <div className="w-full">
              <InputField
                ...
                onChange={(e) =>
                  updateNewParticipant('name', e.target.value)
                }
              />
            </div>
            <div className="w-full">
              <InputField
                ...
                onChange={(e) =>
                  updateNewParticipant('email', e.target.value)
                }
              />
            </div>
          </div>
          ...
          {formData.participants.map((participant, index) => (
            <div className="flex justify-start items-center py-3 px-5 shadow-sm" key={index}>
              ...
              <div className="w-[5%] flex justify-end">
                <IoTrashSharp className='bg-custom-orange-1 text-white p-2 text-3xl rounded-xl cursor-pointer' onClick={() => removeParticipant(index)} />
              </div>
            </div>
          ))}
        </div>
